...
void Screen::fillBGgradient(const uint32_t& top, const uint32_t& bottom) {
    ...
    for (int row = 0; row < height; row++) {
        ...
        uint32_t green = ((colorGetGreen(top) * ((height - row)/height)*100) + (colorGetGreen(bottom) * ((row)/height)*100))/(float)((((height - row)/(float)height)*100)+(((row)/(float)height)*100));
        fill = fill | green;
        fill = fill << 8;
